...
class MineralRadiusTransitionModel(TransitionModel):
    ...
    def harvest(self, s):
        ...
        rover_pos = s[0]
        within = lambda m_pos, r: math.pow(m_pos[0] - rover_pos[0], 2) + math.pow(m_pos[1] - rover_pos[1],
                                                                                  2) <= math.pow(r, 2)
        to_mine = []
        ...
        for i in range(self.mineral_count):
            ...
            if s[1][i] == 0 and (
                    rover_pos == self.mineral_pos[i] or within(self.mineral_pos[i], self.mineral_areas[i])):
                to_mine.append(i)
        if to_mine:
...
class LevelThreeTransitionModel(MineralRadiusTransitionModel):
    ...
    def power_x(self, s, value):
        ...
        s_, r = s, 0
        new_vx = max(-self.rover_max_vel,
                     min(self.rover_max_vel, rover_vel[0] + int(value)))  # velocity bounded by max vel
