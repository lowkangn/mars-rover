...
class MarsRoverDisc(Env):
    def __init__(self, level='1', instance='0'):
        ...
        self.base_env = RDDLEnv.RDDLEnv(domain=f"level {level}/domain.rddl",
                                        instance=f"level {level}/instance{instance}.rddl")
        ...
        self.mineral_pos = list(
            zip(self.base_env.sampler.subs['MINERAL-POS-X'], self.base_env.sampler.subs['MINERAL-POS-Y']))
    ...
    def init_states(self):
        ...
        rover_possible_pos = list(
            itertools.product(np.arange(-self.x_bound, self.x_bound + 1), np.arange(-self.y_bound, self.y_bound + 1)))
...
class LevelThreeEnv(MarsRoverDisc):
    ...
    def init_states(self):
        ...
        rover_possible_vel = list(itertools.product(np.arange(-self.rover_max_vel, self.rover_max_vel + 1),
                                                    np.arange(-self.rover_max_vel, self.rover_max_vel + 1)))
